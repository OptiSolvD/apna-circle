...
const convertUserDataToPDF = async(userData) =>{
    ...
    doc.fontSize(14).text(`Nmae: ${userData.userId.name}`);
...
export const register = async (req,res)=>{
    try{
        ...
        await profile.save();
        return res.json({message:"user ceated "})
    }catch(error){
...
export const login =async (req,res)=>{
    try{
        ...
        if(!isMatch ) return res.status(400).json({message:"invalid crredential"});
...
export const getUserAndProfile = async(req,res) =>{
    try {
        ...
        const {token }= req.query;  // we are sending extra data in the url 
...
console.log("userProfile:", userProfile);
    ...
    return res.json({"message":outputPath});              // doubt in the message
};
...
export const getMyConnectionRequests = async(req,res)=>{
    try{
        ...
        const connection = await ConnectionRequest.find({userId:user._id})
            .populate('connectionId','name username email profilePicture');  //connectionId is also referring the User
...
export const acceptConnectionRequest = async (req, res) => {
    ...
    try {
        ...
        if (action_type === "accept") {
            ...
            const existingReverseConnection =     //we can reflect both side if the one accets the other request under the my network
